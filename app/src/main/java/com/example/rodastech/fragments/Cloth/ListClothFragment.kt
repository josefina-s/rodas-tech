...
class ListClothFragment : Fragment() {
    ...
    override fun onStart() {
        ...
        viewModel.cloths.observe(viewLifecycleOwner) { cloths ->
            ...
            adapter = ClothAdapter(cloths.toMutableList()) { position, buttonId ->
                ...
                when (buttonId) {
                    ...
                    "DETAILS" -> {
                        viewModel.setSelectedCloth(cloths[position])
                        val action =
                            ListClothFragmentDirections.actionListClothFragmentToDetailClothFragment()
            ...
            val sortingOptions = arrayOf("Stock menor a mayor", "Stock mayor a menor")

            binding.spClothList.adapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, sortingOptions)
            binding.spClothList.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                    when(sortingOptions[position]){
                        "Stock menor a mayor" -> adapter.sortClothByActualStockAsc()
                        "Stock mayor a menor" -> adapter.sortClothByActualStockDesc()
                    }
                    binding.recList.adapter=adapter
                }
        ...
        binding.searchCloth.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String?): Boolean {
                val newLista = filterList.filter { cloth ->
                    cloth.name!!.lowercase().contains(newText.toString().lowercase())
                }
